...
public class CuttingMap {
        ...
        while (true) {
            if (StdDraw.isMousePressed()) {  // detected mouse click
                ...
                if (k == included.length + 1) {
                    ...
                    if (cutSites.get(k).size() != 0) included[k] = !included[k];
                    else continue;
                }
                else continue;
    ...
    /**
     * Draws the name and the number of cutting sites in a graph table,
     * and draws the DNA sequence.
     */
    private void drawTable(boolean[] included) {
        seq.draw();
        double length = seq.drawLength();
        StdDraw.setPenColor(StdDraw.BLUE);
        StdDraw.text(length * 0.5, length * 0.6, "Click the name to select or unselect a cutter!");
        int i;
        ...
        StdDraw.rectangle(length * (i % 5 + 0.5) * 0.2, length * 0.5 * (1 - (i / 5) * 0.2), length * 0.1,
                          length * 0.05);
        StdDraw.text(length * (i % 5 + 0.5) * 0.2, length * 0.5 * (1 - (i / 5) * 0.2), "Select All");
        i++;
        StdDraw.rectangle(length * (i % 5 + 0.5) * 0.2, length * 0.5 * (1 - (i / 5) * 0.2), length * 0.1,
                          length * 0.05);
        StdDraw.text(length * (i % 5 + 0.5) * 0.2, length * 0.5 * (1 - (i / 5) * 0.2), "Deselect All");
        StdDraw.setFont();
    }
    
    /**
     * Draws sites of selected cutters against DNA sequence in the graph.
     */
